...
newdata_den <- expand.grid(HASAve = 0, 
  ...
  LFTADen = (seq(1, 50, length.out = 150) - mean(d_raw$LFTADen)) / (2 * sd(d_raw$LFTADen)),
  RemovTreat = c(1, 0),
  y = 0) # ignore y
newdata_hab <- expand.grid(
  # HASAve_raw = seq(1, 5, length.out = 150),
  HASAve = (seq(1, 5, length.out = 150) - mean(d_raw$HASAve)) / (2 * sd(d_raw$HASAve)), 
  LFTADen = 0,
  RemovTreat = c(1, 0),
  y = 0) # ignore y
...
ggplot(est, aes(variable, value, fill = response)) + 
  ...
  geom_violin(position = position_dodge(width = 0.8), alpha = 1, lwd = 0.1, col = NA) +
  coord_flip() +
...
ggsave("figs/tnm-estimates.pdf", width = 5.5, height = 5.5)
```
...
Here's what I see:  *(numbers might have changed slightly)*  *TODO NUMBERS DIFFERENT*
...
pp <- plyr::ldply(pp, function(x) {
  data.frame(X_new, 
    l = apply(x, 2, quantile, probs = 0.25),
    u = apply(x, 2, quantile, probs = 0.75),
...
g <- filter(pp, HASAve == 0) %>% 
  ...
  geom_line(lwd = 1) +
  geom_ribbon(aes(x = LFTADen_raw, ymax = exp(u), ymin = exp(l), fill = `Lionfish removed`), 
...
g
ggsave("figs/predictions-den.pdf", width = 10, height = 3)

g <- filter(pp, LFTADen == 0) %>% 
  ggplot(aes(HASAve_raw, exp(med), colour = `Lionfish removed`)) + 
  facet_wrap(~.id, scales = "free_y") +
  geom_line(lwd = 1) +
  geom_ribbon(aes(x = HASAve_raw, ymax = exp(u), ymin = exp(l), fill = `Lionfish removed`), 
    alpha = 0.15, inherit.aes = FALSE) +
...
g
ggsave("figs/predictions-hab.pdf", width = 10, height = 3)
```
