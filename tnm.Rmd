...
```{r}
X <- model.matrix(dNr ~ 0 + HASAve * RemovTreat + LFTADen * RemovTreat, data = d_sum)
...
```{r}
newdata <- expand.grid(HASAve = 0, 
  LFTADen = seq(min(d_sum$LFTADen), 
    max(d_sum$LFTADen), length.out = 200),
  RemovTreat = c(0.5, -0.5),
  y = 0) # ignore y
X_new <- model.matrix(y ~ 0 + HASAve * RemovTreat + LFTADen * RemovTreat, data = newdata)

fit_tnm <- function(response) {
  f <- paste(response, "~ 0 + HASAve * RemovTreat + LFTADen * RemovTreat")
...
pp <- plyr::ldply(pp, function(x) {
  data.frame(X_new, 
    l = apply(x, 2, quantile, probs = 0.05),
    u = apply(x, 2, quantile, probs = 0.95),
...
b1s <- plyr::ldply(out_cent, function(x) {
  data.frame(b1 = median(extract(x)$beta[,1]))
})

d_sum_long <- left_join(d_sum_long, b1s)
d_sum_long <- mutate(d_sum_long, value_adjusted = value - b1 * HASAve)

cols <- c(RColorBrewer::brewer.pal(4, "Greys")[[3]], RColorBrewer::brewer.pal(4, "Reds")[[3]])
g <- ggplot(pp, aes(LFTADen_raw, exp(med), colour = `Lionfish removed`)) + 
  facet_wrap(~.id, scales = "free_y") +
  geom_line(lwd = 1) +
  geom_ribbon(aes(ymax = exp(u), ymin = exp(l), fill = `Lionfish removed`), 
    alpha = 0.15, lwd = 0) +
  labs(y = "Response value", x = "Lionfish density") +
  scale_color_manual(values = cols) + scale_fill_manual(values = cols) +
  theme_gg() +
  geom_point(data = d_sum_long, aes(x = LFTADen_raw,
    y = exp(value_adjusted), colour = `Lionfish removed`), inherit.aes = FALSE)
# scale_x_log10(breaks = c(1, 2, 4, 6))
g
ggsave("figs/prediction-examples.pdf", width = 9.5, height = 5)
```
